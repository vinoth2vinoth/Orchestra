...
export function getKeyValueStateUrl(env: NodeJS.ProcessEnv = process.env): string | undefined {
    return env.ORCHESTRA_STATE_URL || env.VALKEY_URL || env.REDIS_URL;
}

/**
 * StateAdapter implementation for Redis-compatible key-value protocols.
 *
 * Valkey is the recommended open-source backend for production deployments.
 * The implementation intentionally targets the widely supported protocol so
 * operators can choose Valkey, Redis-compatible services, DragonflyDB, KeyDB,
 * or another compatible backend without changing Orchestra code.
 */
export class KeyValueStateAdapter implements StateAdapter {
    private client: RedisCompatibleClient;

    constructor(url: string = getKeyValueStateUrl() || 'redis://localhost:6379') {
        this.client = new RedisCompatibleClient(url, {
            retryStrategy: (times) => {
        ...
        this.client.on('error', (err) => {
            console.error('KeyValueStateAdapter backend error:', err.message);
        });
    }
